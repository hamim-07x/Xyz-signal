...
export const LockedView: React.FC<LockedViewProps> = ({ telegramId, onSuccess, contactLink, onOpenSupport }) => {
  ...
  return (
    <div className="w-full relative min-h-[350px] flex flex-col items-center justify-center p-1 animate-fade-in-up">
      {/* Container Frame */}
      <div className="absolute inset-0 bg-[#0a0a0a] border border-red-900/50 rounded-2xl shadow-[0_0_30px_rgba(220,38,38,0.1)] overflow-hidden">
         {/* Cyber Lines */}
         ...
         <div className="absolute bottom-0 left-0 w-full h-[2px] bg-gradient-to-r from-transparent via-red-500 to-transparent opacity-50"></div>
         <div className="absolute top-0 left-0 h-full w-[100px] bg-gradient-to-r from-red-900/5 to-transparent skew-x-12"></div>
      </div>

      <div className="relative z-10 w-full max-w-[280px] flex flex-col items-center space-y-6">
        
        {/* Lock Animation */}
        <div className="relative">
            <div className="w-20 h-20 rounded-full border-2 border-red-500/30 flex items-center justify-center animate-pulse">
                <div className="w-16 h-16 rounded-full border border-red-500/50 flex items-center justify-center bg-red-950/20">
        ...
        <div className="text-center space-y-1">
          <h3 className="text-2xl font-bold text-white tracking-[0.2em] uppercase font-mono drop-shadow-[0_0_5px_rgba(220,38,38,0.8)]">SYSTEM LOCKED</h3>
          <p className="text-[10px] text-red-400 font-mono tracking-widest">ENTER DECRYPTION KEY</p>
        </div>
        ...
        <div className="w-full space-y-4">
          <div className="relative group">
            ...
            <input 
                ...
                className="relative w-full bg-black border border-red-800 rounded-lg p-4 text-center text-red-100 font-mono text-lg tracking-[0.2em] focus:outline-none focus:border-red-500 uppercase placeholder-red-900/30"
            />
          </div>

          {error && (
            <div className="text-center text-red-500 text-[10px] font-bold bg-red-950/50 py-2 rounded border border-red-500/30 animate-pulse tracking-wide">
              {error}
            </div>
          )}
          ...
          <button
            ...
            className={`w-full py-4 rounded-lg font-bold flex items-center justify-center space-x-2 transition-all font-mono text-sm tracking-widest uppercase relative overflow-hidden group
              ${loading || !inputKey
          ...
          {(contactLink || onOpenSupport) && (
            <button
              ...
              className="w-full py-2 rounded-lg font-bold flex items-center justify-center space-x-2 transition-all font-mono text-[10px] tracking-widest text-gray-500 hover:text-white border border-transparent hover:border-white/10 hover:bg-white/5"
            >
